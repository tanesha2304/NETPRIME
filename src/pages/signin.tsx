		...
		if (canProceed) {
			mainHttp
				...
				.then(({ authorization, data }: any) => {
					setIsLoading(false);
					localStorage.setItem('NETPRIME-auth', authorization);
					localStorage.setItem('NETPRIME-user', JSON.stringify(data.userDetails));
	...
	return (
		<Scrollbar noDefaultStyles className="main-scrollbar" onScroll={({ scrollTop }: any) => handleOnScroll(scrollTop)}>
			...
			<Form>
				...
				<Form.Text>
					New to NETPRIME? <Form.Link to={ROUTES.SIGNUP.path}>Sign up now</Form.Link>.
				</Form.Text>
